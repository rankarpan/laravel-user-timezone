...
class ImageResizer
{
    ...
    public function upload($type, $input, $name, $crop_dimentions = null)
    {
        ...
        if($crop && !empty($crop_dimentions))
        {
            ...
            if(count($crop_dimentions) == 5)
            {
                ...
                $rotate = $crop_dimentions[4];
                $img->rotate($crop_dimentions[4]);
                $img->crop($crop_dimentions[0], $crop_dimentions[1], $crop_dimentions[2], $crop_dimentions[3]);
            }
            else if(count($crop_dimentions) == 3)
            {
                $width = $crop_dimentions[0];
                $height = $crop_dimentions[1];
                $rotate = $crop_dimentions[4];
        ...
        foreach ($sizes as $key => $s) {
            ...
            switch ($s[2]) {
                case 'stretch':
                    $img->resize($s[0], $s[1], function ($constraint) {
    ...
    public function get($type, $size, $filename)
    {
        ...
        if(file_exists($new_path)){
            return $this->getBaseUrl().'/'.$new_path;
        }
        else if(!file_exists("$original/$filename") && isset($config['types'][$type]['default'])){
            return $this->getBaseUrl().'/'.$config['types'][$type]['default'];
        }
    ...
    */
    private function getBaseUrl() {
        ...
        if(empty($base_url)){
            if (in_array(\App::environment(), $this->config['ignore_environments'])) {
                $this->base_url = \Request::root();
            }
            else if (is_null($this->config['base_url']) || (trim($this->config['base_url']) == '')) {
                $this->base_url = \Request::root();
            } else {
