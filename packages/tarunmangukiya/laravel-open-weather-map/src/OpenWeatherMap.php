...
class OpenWeatherMap
{
     ...
     * @param array $config
     */
    public function __construct($cache, $view, array $config = array())
    {
    ...
    public function generate($options = array())
    {
        ...
        // Get current weather
        $current = $this->getWeather($options['appid'], $options['query'], 0, $options['units'], 1);
        ...
        // Get forecast
        $forecast = $this->getWeather($options['appid'], $options['query'], $options['days'], $options['units']);
    ...
    private function getWeather($appid, $query, $days = 1, $units = 'internal', $type = 0, $lang = 'en')
    {
        $forecast = ($type == 0) ? 'forecast/daily?' : 'weather?';
        return $this->request("http://api.openweathermap.org/data/2.5/{$forecast}{$query}&cnt={$days}&units={$units}&mode=json&lang={$lang}&appid={$appid}");
    }
